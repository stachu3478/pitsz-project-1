...
class LocalLmaxCminSolver(ProperSolver):
        ...
        for j in range(instance.n):
            ...
            for i in range(instance.n):
                ...
                if (c_min is None) or (c < c_min):
                    best_task_c = i
                    #print("Default c " + str(i) + " with " + str(c))
                    c_min = c
            if (current_l_max is None):
                best_task = best_task_c
                current_l_max = c_min - instance.d[best_task_c]
            elif (l_max > current_l_max):
                current_l_max = l_max
